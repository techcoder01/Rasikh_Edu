...
const Header2 = () => {
  ...
  const navLinks = [
    ...
    { href: "/student/institute", text: "Institute" },
    { href: "/student/courses", text: "Courses" },
    { href: "/student/certificate", text: "Certificate" },
    { href: "/student/verification", text: "Verification" },
    { href: "/student/edit-profile", text: "Edit Profile" },
  ];
  ...
  return (
    <>
      <header className="bg-black/60 text-white fixed top-0 left-0 right-0 z-50">
        <nav className="container mx-auto px-4 py-3 flex items-center justify-between max-w-6xl">
            ...
            <div className="hidden md:flex space-x-4">
              {navLinks.map((link, index) => (
                <Link 
                  key={index} 
                  href={link.href} 
                  className={`hover:text-gray-300 py-1 h-fit px-2 rounded-lg transition duration-300 ${pathname === link.href ? 'bg-[#3F4245] text-white' : 'hover:bg-[#3F4245]'}`}
                >
                  {link.text}
                </Link>
              ))}
          ...
          <div className="flex items-center space-x-4">
            ...
            <button className="bg-[#2a2a2a] p-2 rounded-lg">
      ...
      <div className={`fixed inset-y-0 left-0 w-64 bg-[#121212] transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out z-50`}>
